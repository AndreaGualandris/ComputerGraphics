...
public:
	...
	Hit intersect(Ray ray)
	{
		...
		if (D <= radius)
		{
			...
			{
				hit.uv.x = (M_PI + glm::atan(hit.normal.z, hit.normal.x)) / (2.0 / M_PI);
				hit.uv.y = (glm::asin(hit.normal.y) + (M_PI / 2.0)) / M_PI;
			}
		}
		else
		{
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material)
{
	...
	for (int light_num = 0; light_num < lights.size(); light_num++)
	{
		...
		glm::vec3 diffuse;
		glm::vec3 texture = glm::vec3(1.0);

		if (material.texture != NULL)
		{
			texture = material.texture(uv);
		}

		diffuse = texture * material.diffuse * glm::vec3(NdotL);
		glm::vec3 specular = material.specular * glm::vec3(pow(VdotR, material.shininess));

		// float a1 = 1;
		// float a2 = 0.1;
		// float a3 = 0.01;
		// float attenuation = 1 / (a1 + a2 * r + a3 * r * r);

		//for the distance attenuation divide by the square of the distance 



		float r = glm::distance(point, lights[light_num]->position);
		float attenuation = glm::clamp(7.0 / r, 0.0, 1.0);

		//float attenuation = 1 / (glm::distance(point, lights[light_num]->position) * glm::distance(point, lights[light_num]->position));
...
void sceneDefinition()
{
	...
	/* Material textured2;
	textured2.texture = &rcheckerboardTexture;
	objects.push_back(new Sphere(5.0, glm::vec3(6, 4, 23), textured2)); */

	Material ground;
	ground.diffuse = glm::vec3(0.7f, 0.7f, 0.7f);
	ground.ambient = glm::vec3(0.07f, 0.07f, 0.07f);
	ground.specular = glm::vec3(0.6);
	ground.shininess = 3.0;

	objects.push_back(new Plane(glm::vec3(0, -3, (30 - 0.01) / 2), glm::vec3(0, 1, 0), ground));
	objects.push_back(new Plane(glm::vec3(0, 27, (30 - 0.01) / 2), glm::vec3(0, -1, 0), red_specular));
	objects.push_back(new Plane(glm::vec3(-15, 12, (30 - 0.01) / 2), glm::vec3(1, 0, 0), red_specular));
	objects.push_back(new Plane(glm::vec3(15, 12, (30 - 0.01) / 2), glm::vec3(-1, 0, 0), blue_specular));
	objects.push_back(new Plane(glm::vec3(0, 12, 30), glm::vec3(0, 0, -1), green_diffuse));
...
glm::vec3 toneMapping(glm::vec3 intensity)
{
	...
	tonemapped = pow(alpha * pow(intensity, glm::vec3(beta)), glm::vec3(1 / gamma)); // tonemapped intensity
...
int main(int argc, const char *argv[])
{
	...
	int height = 920; // height of the image
