...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material)
{
	...
	for (int light_num = 0; light_num < lights.size(); light_num++)
	{
		...
		// shadow
		Ray shadow_ray(point + 0.001f * light_direction, light_direction);

		for (int i = 0; i < objects.size(); i++)
		{
			Hit hit = objects[i]->intersect(shadow_ray);
...
/**
 ...
 @return Color at the intersection pointtimes
 */
glm::vec3 trace_ray(Ray ray, int n)
{
	...
	if (closest_hit.hit)
	{
		glm::vec3 view_direction = glm::normalize(ray.origin - closest_hit.intersection);

		if (closest_hit.object->getMaterial().reflectivity > 0.0 || closest_hit.object->getMaterial().refractivity > 1.0)
		{
			if ((closest_hit.object->getMaterial().reflectivity > 0.0 && n > 0))
			{
				glm::vec3 reflection = glm::reflect(-view_direction, closest_hit.normal);
				Ray reflection_ray(closest_hit.intersection + 0.001f * reflection, reflection);
				glm::vec3 reflection_color = trace_ray(reflection_ray, n - 1);
				color = reflection_color * closest_hit.object->getMaterial().reflectivity;
			}
			if (closest_hit.object->getMaterial().refractivity > 1.0 && n > 0)
			{
				if (glm::dot(closest_hit.normal, view_direction) > 0.0)
				{
					glm::vec3 refraction = glm::refract(-view_direction, closest_hit.normal, 1.0f / closest_hit.object->getMaterial().refractivity);
					Ray refraction_ray(closest_hit.intersection + 0.001f * refraction, refraction);
					glm::vec3 refraction_color = trace_ray(refraction_ray, n - 1);
					color += refraction_color * (1.0f - closest_hit.object->getMaterial().refractivity);
				}
				else
				{
					glm::vec3 refraction = glm::refract(-view_direction, -closest_hit.normal, closest_hit.object->getMaterial().refractivity);
					Ray refraction_ray(closest_hit.intersection + 0.001f * refraction, refraction);
					glm::vec3 refraction_color = trace_ray(refraction_ray, n - 1);
					color += refraction_color * (1.0f - closest_hit.object->getMaterial().refractivity);
				}
 ...
 */
void sceneDefinition(glm::vec3 position1)
{
	...
	blue_specular.specular = glm::vec3(1.0f);
	blue_specular.shininess = 100.0f;
	blue_specular.reflectivity = 1.0f;

	Material blue_refraction;
	blue_refraction.reflectivity = 0.1f;
	// blue_specular.specular = glm::vec3(1.0f);
	// blue_refraction.shininess = 100.0f;
	blue_refraction.refractivity = 2.0f;

	objects.push_back(new Sphere(1.0, position1, blue_specular));
	objects.push_back(new Sphere(0.5, glm::vec3(-1, -2.5, 6), red_specular));
	// objects.push_back(new Sphere(1.0, glm::vec3(3,-2,6), green_diffuse));
	objects.push_back(new Sphere(2.0f, glm::vec3(-3, -1, 8), blue_refraction));
	...
	lights.push_back(new Light(glm::vec3(0, 26, 5), glm::vec3(0.8f)));
...
int main(int argc, const char *argv[])
{
	...
	if (argc > 3)
	{
		position1 = glm::vec3(atof(argv[2]), atof(argv[3]), atof(argv[4]));
	}
	...
	sceneDefinition(position1); // Let's define a scene
	...
	//stamp position1
	cout << "position1: " << position1.x << " " << position1.y << " " << position1.z << endl;
	...
	{
		std::string filename = "resultsppm/result" + std::to_string(num) + ".ppm";
